...
// 在类的定义中, 仅仅做数据结构的定义. init 方法的定义.
// 然后将各种方法, 添加到 Extension 中, 是一种更加合理的代码组织方式.
...
// 统一的, 将特定的 Header Key 生成编写到一个地方. 使用更加准确和具有表现的函数, 来替代人工进行 key value 书写的过程.
...
// 对于大部分的场景来说, 使用默认值就可以了.
// 所以, 这里提供了几种常见的默认值来进行处理.
...
// 专门定义这样的一个数据结构, 意义就是在这里.
/*
 直接使用 [String: String] 来完成编码任务, 其实是什么问题的.
 但是这种操作原始值的方式, 其实是一直应该警醒的.
 HTTPHeaders, HTTPHeader 的提出, 其实主要的目的是定义各种封装了 字符串 Key 表示的方式, 封装了各种默认值.
 使用这种方式, 可以使得代码可读性, 健壮性大大增加. 
 */
