...
open class SessionDelegate: NSObject {
    ...
    /*
     这个类, 是在 Session 中使用的.
     它的主要目的, 就是充当 URLSession 的 Delegate, 然后传递事件给 Alamofire 的 Session, 更新里面的状态.
     但是, 循环引用是一个不好的行为. 所以, Session 被当做 SessionStateProvider 来进行使用.
     */
    ...
    // 使用, 返回值来充当类型的标识符. 这是在 Swfit 语言中独特的使用方式.
...
extension SessionDelegate: URLSessionTaskDelegate {
    ...
    // 当是 401 的时候, 会走到这里来. Delegate 方法, 会使用 completionHandler 只会 URLSession 内部, 如何进行 Challenge 的处理.
    ...
    open func urlSession(_ session: URLSession,
        ...
        // 从 Session 里面, 找到 Request, 然后调用其中的 UploadProgress 方法. 
