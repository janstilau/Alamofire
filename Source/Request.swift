...
public class Request {
    ...
    // AFN 中, 是将所有的命令对象, 存到了一个专门的 Handler 对象内部.
    // 在 Alamofire 里面, 是将所有的命令对象, 存到了 Request 对象内部.
    struct MutableState {
    ...
    ///
    // 当, 添加了响应回调之后, 会立马进行 真正的 TaskResume 的调用.
    ...
    // 一切的动作, 都是在进行配置.
    // 使用动词作为函数的开头, 然后进行值的配置, 返回 Self. 这种预埋逻辑的使用方式, 非常适合, 异步函数.
