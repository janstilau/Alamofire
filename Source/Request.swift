...
public class Request {
    ...
    func didResume() {
        /*
         dispatchPrecondition(condition:) 是 Swift 中 GCD（Grand Central Dispatch）的一部分，用于在并发编程中检查代码是否在预期的队列或线程上执行。这个函数允许你指定一个条件，这个条件用于验证代码当前是否在特定的队列（例如主队列）或不在该队列上运行。如果条件不满足，程序将触发运行时断言，导致崩溃。
         */
        dispatchPrecondition(condition: .onQueue(underlyingQueue))
